const { getConnection, DB_CONFIG } = require("../../config/database/db-connection");
...
module.exports = {
    ...
    fn: async function (inputs, exits) {
        ...
        for (const [key, value] of Object.entries(prc_params)) {
            prc_params[key] = value ?? null;
        }

        oracledb.initOracleClient({ libDir: process.env.DB_CLENT_PATH });

        let connection = await oracledb.getConnection(DB_CONFIG);

        const result_query = await connection.execute(
            `SELECT query FROM tbl_procedure_declare WHERE name = :prc_name`,
            [prc_name]
        );
        ...
        const result_call = await connection.execute(prc_query, prc_params);
        ...
        const columns = result_data.metaData.map((ele) => ele.name);
